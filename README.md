...
Этот проект представляет собой полноценный **ML-сервис**, предсказывающий вероятность сердечного заболевания по данным пациента или целой тестовой выборки (CSV-файл).
Модель обучена в Jupyter Notebook, включает кастомный препроцессинг, а API реализовано на FastAPI.

Сервис поддерживает два способа получения предсказаний:

* HTML форма → загрузка CSV → ответ в JSON
* REST API POST /upload_csv → JSON
